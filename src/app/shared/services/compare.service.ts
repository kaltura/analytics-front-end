...
export class CompareService implements OnDestroy {
  ...
  private _getCompareValue(compareData: string[], date: moment.Moment, datesDiff: number, reportInterval: KalturaReportInterval): [string, string] {
    ...
    } else if (reportInterval === KalturaReportInterval.hours) {
      relevantDate = DateFilterUtils.getMomentDate(date).subtract(datesDiff);
      relevantLabelString = relevantDate.format('YYYYMMDDHH');
    } else {
  ...
  public compareGraphData(currentPeriod: { from: number, to: number },
    ...
    if (reportInterval === KalturaReportInterval.months) {
      ...
      currentPeriodTitle = `${DateFilterUtils.formatHoursString(currentPeriod.from, analyticsConfig.locale)} – ${DateFilterUtils.formatHoursString(currentPeriod.to, analyticsConfig.locale)}`;
      comparePeriodTitle = `${DateFilterUtils.formatHoursString(comparePeriod.from, analyticsConfig.locale)} – ${DateFilterUtils.formatHoursString(comparePeriod.to, analyticsConfig.locale)}`;
    } else {
      ...
      const getFormatter = colors => params => {
        ...
        if (currentPeriodDate && comparePeriodDate) {
          ...
          if (reportInterval === KalturaReportInterval.months) {
            ...
            currentPeriod = DateFilterUtils.formatHoursString(currentPeriod);
            comparePeriod = DateFilterUtils.formatHoursString(comparePeriod);
          } else {
  ...
  public compareToMetric(config: ReportDataItemConfig,
    ...
    const createFunc = func => series => parseFloat(func(...[].concat.apply([], series.map(({ data }) => data))).toFixed(2));
    ...
    const currentInterval = parseFloat(((currentMax - currentMin) / 5).toFixed(3));
    const compareInterval = parseFloat(((compareMax - compareMin) / 5).toFixed(3));
